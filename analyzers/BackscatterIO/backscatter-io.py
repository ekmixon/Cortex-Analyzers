...
class BackscatterAnalyzer(Analyzer):
    ...
    def run(self):
        ...
        if self.data_type == "ip":
            kwargs['query_type'] = 'ip'
        elif self.data_type == "network":
            kwargs['query_type'] = 'network'
        elif self.data_type == 'autonomous-system':
            kwargs['query_type'] = 'asn'
        elif self.data_type == 'port':
            kwargs['query_type'] = 'port'
        else:
            self.notSupported()
            return False
    ...
    def summary(self, raw):
        ...
        if self.service == 'observations':
            summary = raw.get('results', {}).get('summary', {})
            taxonomies += [
                self.build_taxonomy(
                    level,
                    namespace,
                ...
                self.build_taxonomy(
                    ...
                    namespace,
                    'IP Addresses',
                    summary.get('ip_address_count', 0),
                ),
                self.build_taxonomy(
                ...
                self.build_taxonomy(
                    ...
                    namespace,
                    'AS',
                    summary.get('autonomous_system_count', 0),
                ),
                self.build_taxonomy(
            ...
            if self.data_type == 'ip':
                ...
                for result in raw.get('results', []):
                    display = f"{result.get('service')} ({result.get('protocol')})"
